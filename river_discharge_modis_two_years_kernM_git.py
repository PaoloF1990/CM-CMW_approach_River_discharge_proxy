...
@description: Python code to extract a proxy of river discharge from reflectance 
            data of MODIS using GEE platform
@date: 29/08/2024
@version: 1
...
while attempt==1:
    try:
        ...
        try:
            ...
            snowMOD=ee.ImageCollection("MODIS/006/MOD10A1").select('NDSI_Snow_Cover').filterDate(ee.Date('2003-01-01'),ee.Date('2020-01-01')).filterBounds(region2)
            snowMYD=ee.ImageCollection("MODIS/006/MYD10A1").select('NDSI_Snow_Cover').filterDate(ee.Date('2003-01-01'),ee.Date('2020-01-01')).filterBounds(region2)
        ...
        for prod in products:
            product1='MODIS/006/'+prod+'09GQ';
            product2='MODIS/006/'+prod+'09GA';
                

            if os.path.isfile('data_full_'+product1[n0:]+'_'+str2+'.mat'):
                ...
                try:
                    Wat=ee.Image(folderGEE+'/Wat_'+product1[n0:]+'_'+str2+'_ray_'+ray0+'_bitmask_'+str(bitmask)).reproject(crs_mod,trans_mod)
                    Wat.getInfo()
                    
                    Wat2=ee.Image(folderGEE+'/Wat2_'+product1[n0:]+'_'+str2+'_ray_'+ray0+'_bitmask_'+str(bitmask)).reproject(crs_mod,trans_mod)
                    Wat2.getInfo()
    
                except:
                    if self_Wat==0:
                        Wat=ee.Image('JRC/GSW1_4/GlobalSurfaceWater').select('max_extent')
                        projW=Wat.projection();
                        scale_W=ee.Number(projW.nominalScale()).getInfo()
                        trans_W=projW.getInfo()['transform'];
                        crs_W=projW.getInfo()['crs'];
                        Wat2=Wat.reduceNeighborhood(ee.Reducer.mean(), ee.Kernel.square(1-nar,'pixels')).reproject(crs_W,trans_W).eq(1).reproject(crs_W,trans_W)
                        Wat=Wat.reduceNeighborhood(ee.Reducer.mean(), ee.Kernel.square(4,'pixels')).reproject(crs_W,trans_W).gt(0).reproject(crs_W,trans_W);
            
                        vectors = Wat2.reduceToVectors(**{
                          'geometry': region,
                          'crs': crs_W,
                          'crsTransform': trans_W,
                          'geometryType': 'polygon',
                          'eightConnected': False,
                          'labelProperty': 'zone'});
                        def river_select(feat):
                          ins=feat.intersects(ee.Geometry.MultiPoint(coordW),1,projW) 
                          return feat.set('inside',ins)
    
                        
                        solv=vectors.map(river_select)
                        solv=solv.filter(ee.Filter.eq('inside',True))
                        Wat2=Wat2.clip(solv).unmask(0).reduceNeighborhood(ee.Reducer.mean(), ee.Kernel.square(5-nar,'pixels')).reproject(crs_W,trans_W).gt(0).reproject(crs_mod,trans_mod)
                        
                        task = ee.batch.Export.image.toAsset(image=Wat,
                                                         description='Wat_'+product1[n0:]+'_'+str2+'_ray_'+ray0+'_bitmask_'+str(bitmask),
                                                         region=region,
                                                         assetId= folderGEE+'/Wat_'+product1[n0:]+'_'+str2+'_ray_'+ray0+'_bitmask_'+str(bitmask),
                                                         crs=crs_mod,
                                                         crsTransform=trans_mod,
                                                         maxPixels=1000000000)
                        task2 = ee.batch.Export.image.toAsset(image=Wat2,
                                                         description='Wat2_'+product1[n0:]+'_'+str2+'_ray_'+ray0+'_bitmask_'+str(bitmask),
                                                         region=region,
                                                         assetId= folderGEE+'/Wat2_'+product1[n0:]+'_'+str2+'_ray_'+ray0+'_bitmask_'+str(bitmask),
                                                         crs=crs_mod,
                                                         crsTransform=trans_mod,
                                                         maxPixels=1000000000)
                        task.start()
                        ...
                        Wat=ee.Image('JRC/GSW1_2/GlobalSurfaceWater').select('max_extent')
                        projW=Wat.projection();
                        scale_W=ee.Number(projW.nominalScale()).getInfo()
                        trans_W=projW.getInfo()['transform'];
                        crs_W=projW.getInfo()['crs'];
                        l=l.buffer(4*scale_W)
                        im=ee.Image(1).reproject(crs_W,trans_W).clip(l)
                        im=ee.Image(1).updateMask(im).unmask(0).reproject(crs_W,trans_W).gt(0).reproject(crs_mod,trans_mod)
                ...
                def selection(image):
                    ...
                    im=image.select(maskband).reproject(crs_mod,trans_mod).bitwiseAnd((bitmask))
                    image=image.select(band).multiply(mult).updateMask(im.gt(0).And(im.neq(3)).Not())
                    image=image.updateMask(image.gt(0))
                    
                    value=image.select(band[0]).mask().updateMask(Wat).reduceRegion(**{'reducer':ee.Reducer.mean(),'geometry':region,'crs':crs_mod,'crsTransform':trans_mod,'bestEffort':True})
                    value=ee.Number(value.get(value.keys().get(0)))
                    return image.set('Valid_perc',value).set('system:time_start',date)
                
                # FILTERING VALID IMAGES WITH RIVER VALID PIXEL PERCENTAGE
                ...
                try:
                    unvalid=ee.Image(folderGEE+'/Unvalid_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)).reproject(crs_mod,trans_mod)
                    unvalid.getInfo()
                except:
                    unvalid=(ee.Image(coll0.count()).divide(ee.Number(coll0.size())).gt(0.05)).select(band[0])

                    task = ee.batch.Export.image.toAsset(image=unvalid,
                                                     description='Unvalid_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask),
                                                     region=region,
                                                     assetId= folderGEE+'/Unvalid_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask),
                                                     crs=crs_mod,
                                                     crsTransform=trans_mod,
                                                     maxPixels=1000000000)
                    task.start()
                ...
                def mosaic(nday):
                    image=ee.ImageCollection(product1).filterDate(ee.Date(nday),ee.Date(ee.Number(nday).add(86400000))).filterBounds(region).select(band[0]).first()#.reduce(ee.Reducer.mean()).rename(band[0]).reproject(crs_mod,trans_mod)
                    ...
                    image2=image2.bitwiseAnd((bitmask)).reproject(crs_mod,trans_mod)
                    image=image.multiply(mult).updateMask(image2.gt(0).And(image2.neq(3)).Not().And(unvalid.select(band[0])))
                    image=image.updateMask(image.gt(0))
                ...
                def mosaicNDVI(nday):
                    image=ee.ImageCollection(product1).filterDate(ee.Date(nday),ee.Date(ee.Number(nday).add(86400000))).filterBounds(region).select(band).first()#.reduce(ee.Reducer.mean()).rename(band).reproject(crs_mod,trans_mod)
                    image2=ee.ImageCollection(product2).filterDate(ee.Date(nday),ee.Date(ee.Number(nday).add(86400000))).filterBounds(region).select(maskband).first()#.reduce(ee.Reducer.mean()).rename(maskband).reproject(crs_mod,trans_mod)
                    image2=image2.bitwiseAnd((bitmask)).reproject(crs_mod,trans_mod)
                    image=image.multiply(mult).updateMask(image2.gt(0).And(image2.neq(3)).Not().And(unvalid))
                    image=image.updateMask(image.gt(0))
                ...
                if extr_video==1:
                    ...
                    task = ee.batch.Export.video.toDrive(**{
                        ...
                        'description': ('video_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)),
                        'crs': crs_mod,
                        'framesPerSecond': 10,
                        'crsTransform':trans_mod,
                        'region': region
                ...
                print('C extraction')
                try:
                    C2mask=ee.Image(folderGEE+'/Cmask_2_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)).reproject(crs_mod,trans_mod)                    
                    C2mask.getInfo()
                    ...
                    task1 = ee.batch.Export.image.toAsset(image=C2mask,
                                                     description='Cmask_2_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask),
                                                     region=region,
                                                     assetId= folderGEE+'/Cmask_2_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask),
                                                     crs=crs_mod,
                                                     crsTransform=trans_mod,
                                                     maxPixels=1000000000)
                    task1.start()
                ...
                if veg==1:
                    print('V extraction')
                    try:
                        Vmask=ee.Image(folderGEE+'/Vmask_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)).reproject(crs_mod,trans_mod)
                        Vmask.getInfo()
                    except:
                        ...
                        task2 = ee.batch.Export.image.toAsset(image=Vmask,
                                                     description='Vmask_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask),
                                                     region=region,
                                                     assetId= folderGEE+'/Vmask_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask),
                                                     crs=crs_mod,
                                                     crsTransform=trans_mod,
                                                     maxPixels=1000000000)
                        task2.start()
                ...
                if pix_W1==1:        
                    print('W1 extraction')
                    try:
                        Wmask=ee.Image(folderGEE+'/Wmask_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)).reproject(crs_mod,trans_mod)
                        Wmask.getInfo()
                    except:
                        ...
                        task3 = ee.batch.Export.image.toAsset(image=Wmask,
                                                         description='Wmask_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask),
                                                         region=region,
                                                         assetId= folderGEE+'/Wmask_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask),
                                                         crs=crs_mod,
                                                         crsTransform=trans_mod,
                                                         maxPixels=1000000000)
                        task3.start()
                ...
                if pix_W2==1:                        
                    print('W2 extraction')
                    try:
                        Wmask2=ee.Image(folderGEE+'/Wmask2_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)).reproject(crs_mod,trans_mod)
                        Wmask2.getInfo()
                    except:
                        ...
                        task4 = ee.batch.Export.image.toAsset(image=Wmask2,
                                                         description='Wmask2_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask),
                                                         region=region,
                                                         assetId= folderGEE+'/Wmask2_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask),
                                                         crs=crs_mod,
                                                         crsTransform=trans_mod,
                                                         maxPixels=1000000000)
                        task4.start()
                ...
                if no_veg==1:                
                    if pix_Mcal_k1_CM==1: 
                        ...
                        print('M CM cal k1 extraction')                     
                        try:
                            Mmask_C2M_k1=ee.Image(folderGEE+'/Mmask_C2_calib_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1)).reproject(crs_mod,trans_mod)
                            ...
                            task1 = ee.batch.Export.image.toAsset(image=Mmask_C2M_k1,
                                                                 description='Mmask_C2_calib_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1),
                                                                 region=region,
                                                                 assetId= folderGEE+'/Mmask_C2_calib_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1),
                                                                 crs=crs_mod,
                                                                 crsTransform=trans_mod)
                            task1.start()                          
                    ...
                    if pix_Mcal_k2_CM==1: 
                        ...
                        try:
                            Mmask_C2M_k2=ee.Image(folderGEE+'/Mmask_C2_calib_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2)).reproject(crs_mod,trans_mod)
                            ...
                            task2 = ee.batch.Export.image.toAsset(image=Mmask_C2M_k2,
                                                                 description='Mmask_C2_calib_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2),
                                                                 region=region,
                                                                 assetId= folderGEE+'/Mmask_C2_calib_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2),
                                                                 crs=crs_mod,
                                                                 crsTransform=trans_mod)
                            task2.start()
                    ...
                    if pix_Mcal_k1_CMW==1 and pix_W1==1: 
                        ...
                        try:
                            Mmask_C2W1M_k1=ee.Image(folderGEE+'/Mmask_C2_calib_W1_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1)).reproject(crs_mod,trans_mod)
                            ...
                            task3 = ee.batch.Export.image.toAsset(image=Mmask_C2W1M_k1,
                                                                 description='Mmask_C2_calib_W1_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1),
                                                                 region=region,
                                                                 assetId= folderGEE+'/Mmask_C2_calib_W1_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1),
                                                                 crs=crs_mod,
                                                                 crsTransform=trans_mod)
                            task3.start()
                    ...
                    if pix_Mcal_k1_CMW==1 and pix_W2==1: 
                        ...
                        try:
                            Mmask_C2W2M_k1=ee.Image(folderGEE+'/Mmask_C2_calib_W2_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1)).reproject(crs_mod,trans_mod)
                            ...
                            task4 = ee.batch.Export.image.toAsset(image=Mmask_C2W2M_k1,
                                                                 description='Mmask_C2_calib_W2_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1),
                                                                 region=region,
                                                                 assetId= folderGEE+'/Mmask_C2_calib_W2_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1),
                                                                 crs=crs_mod,
                                                                 crsTransform=trans_mod)
                            task4.start()
                    ...
                    if pix_Mcal_k2_CMW==1 and pix_W1==1: 
                        ...
                        try:
                            Mmask_C2W1M_k2=ee.Image(folderGEE+'/Mmask_C2_calib_W1_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2)).reproject(crs_mod,trans_mod)
                            ...
                            task5 = ee.batch.Export.image.toAsset(image=Mmask_C2W1M_k2,
                                                                 description='Mmask_C2_calib_W1_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2),
                                                                 region=region,
                                                                 assetId= folderGEE+'/Mmask_C2_calib_W1_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2),
                                                                 crs=crs_mod,
                                                                 crsTransform=trans_mod)
                            task5.start()
                    ...
                    if pix_Mcal_k2_CMW==1 and pix_W2==1: 
                        ...
                        try:
                            Mmask_C2W2M_k2=ee.Image(folderGEE+'/Mmask_C2_calib_W2_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2)).reproject(crs_mod,trans_mod)
                            ...
                            task6 = ee.batch.Export.image.toAsset(image=Mmask_C2W2M_k2,
                                                                 description='Mmask_C2_calib_W2_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2),
                                                                 region=region,
                                                                 assetId= folderGEE+'/Mmask_C2_calib_W2_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2),
                                                                 crs=crs_mod,
                                                                 crsTransform=trans_mod)
                            task6.start()
                ...
                if veg==1:                
                    if pix_Mcal_k1_CM==1: 
                        # calculate calibrated CVM M k1 pixel
                        print('M CVM k1 cal extraction')                     
                        try:
                            Mmask_C2VM_k1=ee.Image(folderGEE+'/Mmask_C2V_calib_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1)).reproject(crs_mod,trans_mod)
                            ...
                            task7 = ee.batch.Export.image.toAsset(image=Mmask_C2VM_k1,
                                                                 description='Mmask_C2V_calib_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1),
                                                                 region=region,
                                                                 assetId= folderGEE+'/Mmask_C2V_calib_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1),
                                                                 crs=crs_mod,
                                                                 crsTransform=trans_mod)
                            task7.start()
        
                    if pix_Mcal_k2_CM==1: 
                        # calculate calibrated CVM M k2 pixel
                        print('M CVM k2 cal extraction')                     
                        try:
                            Mmask_C2VM_k2=ee.Image(folderGEE+'/Mmask_C2V_calib_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2)).reproject(crs_mod,trans_mod)
                            ...
                            task8 = ee.batch.Export.image.toAsset(image=Mmask_C2VM_k2,
                                                                 description='Mmask_C2V_calib_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2),
                                                                 region=region,
                                                                 assetId= folderGEE+'/Mmask_C2V_calib_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2),
                                                                 crs=crs_mod,
                                                                 crsTransform=trans_mod)
                            task8.start()
                                                   
                    if pix_Mcal_k1_CMW==1 and pix_W1==1: 
                        # calculate calibrated CVMW1 M k1 pixel
                        print('M CVMW1 k1 cal extraction')                     
                        try:
                            Mmask_C2VW1M_k1=ee.Image(folderGEE+'/Mmask_C2V_calib_W1_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1)).reproject(crs_mod,trans_mod)
                            ...
                            task9 = ee.batch.Export.image.toAsset(image=Mmask_C2VW1M_k1,
                                                                 description='Mmask_C2V_calib_W1_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1),
                                                                 region=region,
                                                                 assetId= folderGEE+'/Mmask_C2V_calib_W1_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1),
                                                                 crs=crs_mod,
                                                                 crsTransform=trans_mod)
                            task9.start()
        
                    if pix_Mcal_k1_CMW==1 and pix_W2==1: 
                        # calculate calibrated CVMW2 M k1 pixel
                        print('M CMW2 k1 cal extraction')                     
                        try:
                            Mmask_C2VW2M_k1=ee.Image(folderGEE+'/Mmask_C2V_calib_W2_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1)).reproject(crs_mod,trans_mod)
                            ...
                            task10 = ee.batch.Export.image.toAsset(image=Mmask_C2VW2M_k1,
                                                                 description='Mmask_C2V_calib_W2_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1),
                                                                 region=region,
                                                                 assetId= folderGEE+'/Mmask_C2V_calib_W2_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern1),
                                                                 crs=crs_mod,
                                                                 crsTransform=trans_mod)
                            task10.start()
                                                
                    if pix_Mcal_k2_CMW==1 and pix_W1==1: 
                        # calculate calibrated CVMW1 M k2 pixel
                        print('M CVMW1 k2 cal extraction')                     
                        try:
                            Mmask_C2VW1M_k2=ee.Image(folderGEE+'/Mmask_C2V_calib_W1_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2)).reproject(crs_mod,trans_mod)
                            ...
                            task11 = ee.batch.Export.image.toAsset(image=Mmask_C2VW1M_k2,
                                                                 description='Mmask_C2V_calib_W1_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2),
                                                                 region=region,
                                                                 assetId= folderGEE+'/Mmask_C2V_calib_W1_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2),
                                                                 crs=crs_mod,
                                                                 crsTransform=trans_mod)
                            task11.start()
                   
                    if pix_Mcal_k2_CMW==1 and pix_W2==1: 
                        # calculate calibrated CVMW2 M k2 pixel
                        print('M CVMW2 k2 cal extraction')                     
                        try:
                            Mmask_C2VW2M_k2=ee.Image(folderGEE+'/Mmask_C2V_calib_W2_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2)).reproject(crs_mod,trans_mod)
                            ...
                            task12 = ee.batch.Export.image.toAsset(image=Mmask_C2VW2M_k2,
                                                                 description='Mmask_C2V_calib_W2_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2),
                                                                 region=region,
                                                                 assetId= folderGEE+'/Mmask_C2V_calib_W2_VD_'+product1[n0:]+'_'+str2+'_'+d1+'_'+d2+'_ray_'+ray0+'_bitmask_'+str(bitmask)+'_var_'+hydro+'_k'+str(kern2),
                                                                 crs=crs_mod,
                                                                 crsTransform=trans_mod)
                            task12.start()
                ...
                for k in range(len(years)-1):
                    ...
                    coll=ee.ImageCollection(product1).filterDate(d01,d02).filterBounds(region).select(band);
                    
                    coll2=ee.ImageCollection(product2).filterDate(d01,d02).filterBounds(region).select(maskband);
                    coll=coll.combine(coll2)
                    print('original length: '+str(coll.size().getInfo()))
                    coll0=coll.map(selection)
                    ...
                    def mosaic2(nday):
                        ...
                        image2=image2.bitwiseAnd((bitmask)).reproject(crs_mod,trans_mod)
                        image=image.multiply(mult).updateMask(image2.gt(0).And(image2.neq(3)).Not())
                        image=image.updateMask(image.gt(0))
                ...
                Dmat=np.array([d.toordinal()+366 for d in D])  #Transform the dates in matlab format
                ...
                savemat('data_full_'+product1[n0:]+'_'+str2+'.mat',dic)
